...
export default function Game() {
  ...
  const moves = history.map((squares, move) => {
    ...
    return (
      <li
        key={move}
        className="py-[2px] rounded-md bg-yellow-200 hover:bg-yellow-300 duration-300"
      >
        <button onClick={() => jumpHistory(move)}>{description}</button>
      </li>
    );
  });
    ...
    <div className="bg-stone-900 w-full h-screen flex justify-center items-center">
      <div className="flex space-x-9">
        ...
        <div className="w-44 text-center rounded-lg bg-stone-700">
          <div className="pt-4 pb-3 text-yellow-200 font-semibold">
            Game History
          </div>
          <ul className="text-gray-800 space-y-3 text-sm px-4">{moves}</ul>
        </div>
      </div>
    </div>
  );
